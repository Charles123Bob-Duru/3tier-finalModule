...
resource "aws_instance" "app1" {
  ...
  subnet_id              = local.private_subnet_id[0] # private subnet
...
resource "aws_instance" "app2" {
  ...
  ami                    = data.aws_ami.ami.id
  instance_type          = "t2.micro"
  subnet_id              = local.private_subnet_id[1] # private subnet
  vpc_security_group_ids = [aws_security_group.fronend_app.id]
...
resource "aws_instance" "registration_app" {
  ...
  subnet_id              = element(slice(local.private_subnet_id, 0, 2), count.index) # ensure infra stability...
